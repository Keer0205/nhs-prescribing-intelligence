...
icb_filter_and = "" if selected_icb == "All ICBs" else f"AND icb_name = '{selected_icb}'"
...
with t2:
    ...
    amr = q(f"SELECT practice_name, icb_name, antibiotic_rate_pct, total_cost FROM summary_amr {icb_filter} ORDER BY antibiotic_rate_pct DESC")
...
with t3:
    ...
    anomaly = q(f"SELECT practice_name, icb_name, bnf_chapter_plus_code, total_cost, avg_cost, rx_count FROM summary_anomaly {icb_filter} ORDER BY total_cost DESC LIMIT 50")
